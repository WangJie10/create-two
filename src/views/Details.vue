<template>
  <div id="details">
    ...
    <div class="main">
      ...
      <div class="block">
        <div>
          <img style="max-height:560px;" :src="$target + productDetails.productPicture"/>
        </div>
      </div>
      <!-- 左侧商品轮播图END -->
    ...
    <div class="main">
      <h3 id="pingjia">用户评价</h3>
      <el-divider></el-divider>
      <ul>
        <li v-for="comment in productComments" v-bind:key="comment.id"
            style="border-bottom: 1px lavender dashed;padding: 15px 0">
          <div style="display: flex;justify-content: space-between">
            <div>{{ comment.username }} <span style="font-size: 12px;color: #b0b0b0">{{ comment.createTime }}</span>
            </div>
        ...
        <li v-if="productComments.length===0">
          暂无评价
        </li>
...
export default {
  ...
  methods: {
    ...
    addHistory() {
      this.$axios.post("/api/history/user/" + this.productID + "/" + this.$store.getters.getUser.userId)
    },
    // 加入购物车
    addShoppingCart() {
      // 判断是否登录,没有登录则显示登录组件
    ...
    getProductComments(val) {
      this.$axios
          .get("/api/productComment/list/" + val)
          .then(res => {
    ...
    toPingjia() {
      document.querySelector("#pingjia").scrollIntoView(true);
    }
  }
}
</script>
